...
import (
	...
	"github.com/jon/alfred-letterboxd/alfred"
...
func main() {
	...
	case "people":
		searchPeople(query)
	default:
		alfred.OutputError("Unknown command: %s", searchType)
		return
	}
}

func searchFilms(query string) {
	films, err := letterboxd.SearchFilms(query, 10)
	if err != nil {
		alfred.OutputError("Failed to search films: %v", err)
		return
	}
...
func searchPeople(query string) {
	people, err := letterboxd.SearchPeople(query, 10)
	if err != nil {
		alfred.OutputError("Failed to search people: %v", err)
		return
	}
