...
func SearchFilms(query string, limit int) ([]Film, error) {
	...
	if err != nil {
		...
		return searchFilmsUncached(query, limit)
	}
	...
	films, err = searchFilmsUncached(query, limit)
	if err != nil {
		return nil, err
	}
...
func searchFilmsUncached(query string, limit int) ([]Film, error) {
	client := NewClient()

	// URL encode the query and construct search URL
	encodedQuery := url.PathEscape(strings.ToLower(strings.ReplaceAll(query, " ", "+")))
	searchURL := fmt.Sprintf("https://letterboxd.com/s/search/films/%s/", encodedQuery)
	...
	// Find film results - they're in a list structure
